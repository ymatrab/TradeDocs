...
| --- | --- | --- |
| All foundational components render at mobile/tablet/desktop breakpoints | Token system in `src/app/globals.css`; primitives in `src/components/primitives`; showcase route `/_design` renders every component and state | `design-system.spec.ts` screenshots at 360/768/1440 |
...
| Automated axe checks on affected pages | `design-system.spec.ts` axe runs on `/` and `/_design` |
...
| Tenant isolation for every new data path | Not applicable: Task 03 introduces no data path. Authorization failure is covered by the production gating of `/_design`. Tenant isolation begins at Task 04. |
