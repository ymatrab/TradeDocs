...
for (const [path, metadata] of Object.entries(lock.packages)) {
  ...
  const license =
    metadata.license ??
    (existsSync(manifestPath) ? JSON.parse(readFileSync(manifestPath, 'utf8')).license : undefined);
  if (!allowed.has(license) && !exceptions[`${path}@${metadata.version}`])
    issues.push(`${path}@${metadata.version}: ${license ?? 'missing license metadata'}`);
}
if (issues.length) {
