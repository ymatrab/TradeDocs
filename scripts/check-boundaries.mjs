...
for (const file of files('src').filter((path) => /\.[tj]sx?$/.test(path))) {
  ...
  if (
    ...
    /(?:SUPABASE_SERVICE_ROLE|RESEND_API_KEY|TURNSTILE_SECRET|PAYMENT_WEBHOOK_SECRET|from ['"]@\/lib\/(?:config\/server|supabase\/server|security\/rate-limit))/.test(
      text,
    )
  ) {
...
if (failed) process.exitCode = 1;
  ...
  console.log(
    'Static server/client boundary checks passed; the Next build additionally enforces server-only imports.',
  );
